...
local Filger = {}
local MyUnits = {player = true, pet = true}
...
function Filger:UnitBuff(unitID, inSpellID, spn, absID)
	for i = 1, 40 do
		local name, rank, icon, count, duration, expirationTime = UnitBuff(unitID, i)
		if name == spn then
			local unitCaster
			if not (duration == nil and expirationTime == nil) then
				unitCaster = "player"
			else
				unitCaster = nil
			end
			local spellID = inSpellID
...
function Filger:UnitDebuff(unitID, inSpellID, spn, absID)
	for i = 1, 40 do
		local name, rank, icon, count, debuffType, duration, expirationTime = UnitDebuff(unitID, i)
		if name == spn then
			local unitCaster
			if not (duration == nil and expirationTime == nil) then
				unitCaster = "player"
			...
			local spellID = inSpellID
			return name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, spellID
		end
	end
end
		...
		if not bar then
			...
			if self.Mode == "ICON" then
				if bar.cooldown then
					...
					bar.cooldown = CreateFrame("Cooldown", "$parentCD", bar, "oUF_CooldownFrameTemplate")
		...
		for i = 1, #C["filger_spells"][T.class][id], 1 do
			...
			if data.filter == "BUFF" then
				...
				if spn then
					...
					name, _, icon, count, duration, expirationTime, caster, spid = Filger:UnitBuff(data.unitID, data.spellID, spn, data.absID)
					if name and (data.caster ~= 1 and (caster == data.caster or data.caster == "all") or MyUnits[caster]) then
						if not data.count or count >= data.count then
							...
							if duration then
								start = GetTime() - (duration - expirationTime)
							end
							found = true
						end
					end
			...
			elseif data.filter == "DEBUFF" then
				...
				if spn then
					...
					name, _, icon, count, _, duration, expirationTime, caster, spid = Filger:UnitDebuff(data.unitID, data.spellID, spn, data.absID)
					if name and (data.caster ~= 1 and (caster == data.caster or data.caster == "all") or MyUnits[caster]) then
						-- start = expirationTime - duration
						if duration then
							start = GetTime() - (duration - expirationTime)
			...
			elseif data.filter == "ICD" then
				if data.trigger == "BUFF" then
					...
					if spn then
						...
						name, _, _, _, _, _, _, spid = Filger:UnitBuff("player", data.spellID, spn, data.absID)
					end
				elseif data.trigger == "DEBUFF" then
					...
					if spn then
						...
						name, _, _, _, _, _, _, _, spid = Filger:UnitDebuff("player", data.spellID, spn, data.absID)
					end
