...
----------------------------------------------------------------------------------------
--	Fog of war on World Map (module from Mapster by Nevcairiel)
----------------------------------------------------------------------------------------
local errata = {
	-- Eastern Kingdoms
	["Alterac"] = {
...
local function UpdateOverlayTextures(frame, frameName, textureCache, scale, alphaMod)
	...
	for texName, texID in pairs(overlayMap) do
		...
		local textureWidth, textureHeight, offsetX, offsetY = math.fmod(texID, 2^10), math.fmod(floor(texID / 2^10), 2^10), math.fmod(floor(texID / 2^20), 2^10), floor(texID / 2^30)
		...
		for j = 1, numTexturesTall do
			if j < numTexturesTall then
				...
				texturePixelHeight = math.fmod(textureHeight, 256)
			...
			for k = 1, numTexturesWide do
				...
				if k < numTexturesWide then
					...
					texturePixelWidth = math.fmod(textureWidth, 256)
	...
	table.wipe(discoveredOverlays)
end

local function UpdateWorldMapOverlays()
	if not WorldMapFrame:IsShown() then return end

	if NUM_WORLDMAP_OVERLAYS > NUM_WORLDMAP_OVERLAYS then
	...
	UpdateOverlayTextures(WorldMapDetailFrame, "FogWorldMapOverlay%d", worldMapCache, 1, 0)
end

hooksecurefunc("WorldMapFrame_Update", UpdateWorldMapOverlays)

table.wipe(worldMapCache)
